...
- **TMDB는 살아 있다 — "폐기" 서술은 2026-08-02부로 철회됐다.** API 키를 발급받았고 인앱 포스터 검색이 #537(열림)로 서 있다. 이슈 본문이 Seed spec `docs/specs/tmdb-poster-search.yaml`을 언급하지만 그 파일은 레포에 커밋된 적 없음(2026-08-02 확인) — 착수 전 재생성하거나 이슈 본문 인용을 정정할 것. KOBIS가 포스터를 안 주는 게 그 근거고(`docs/KOBIS_API.md`), 착수 시 열린 질문 1번(TMDB 포스터는 100% 2:3 세로인데 `stub`·`35mm-landscape`는 3:2 가로 슬롯 — #529 프리셋과 정면 충돌)을 먼저 정할 것. **폐기가 유지되는 건 Supabase뿐이다.**
- **Next Up**: #631(포스터를 선택 사항으로) 닫힘, PR #632 머지 완료(2026-08-02) — `_shared.tsx`의 `data-poster-root` 분기 + `canExport`에서 `hasPoster` 게이트 제거(D3)까지 전부 main에 있다. 이게 선행 조건이던 진입 동선 재설계 에픽 #612가 이제 착수 가능하고, Seed spec `docs/specs/landing-mood-first-ocr-entry.yaml`이 1차 범위를 #615(히어로 캐러셀 전시 → 선택 재설계)·#635(OCR을 랜딩 주 CTA로 승격 + 이탈 경로 3종)로 확정해뒀다. #537(TMDB 포스터 검색)은 그 스펙의 2차 범위(OCR 제목 → 자동 검색 체인)로 순서가 밀렸다.
