...
### 📏 크롬 측정 하네스 (400×675)
- **저장물(export JPEG)을 픽셀로 재려면 `scripts/capture-export.mjs`**(#506). `--material/--coating/--intensity/--out`으로 후가공 저장물을 파일로 떨어뜨리고, `--compare a.jpg b.jpg [--diff-out d.png]`가 채널 최대 절대차·넘침 bbox·×20 증폭 diff를 낸다(디코더는 Chrome 자신 = 새 의존성 0). 하네스 잡음 바닥은 같은 트리 2회 캡처 기준 maxΔ 2라 허용치는 3.
  - **헤드리스에서 '사진에 저장'이 안 끝나는 건 앱 버그가 아니다.** macOS 헤드리스 Chrome이 `navigator.canShare({files})`에 **true**를 줘서 `ResultPanel`이 `shareTicketAsJpeg` 경로를 타는데, OS 공유 시트가 못 뜨니 `navigator.share`의 Promise가 영영 settle하지 않아 CTA가 "저장 중..."에 묶인다 — 캡처는 그 전에 이미 끝나 있다(`?debug=1` 로그 `[capture:main] out=…`이 t<3s). 그래서 이 하네스는 `canShare`를 false로 스텁해 다운로드 경로로 떨어뜨리고, 이미 있던 `capture-debug-result` 이벤트로 바이트를 회수한다. **Web Share 게이트를 타는 흐름을 헤드리스로 재는 새 스크립트는 전부 같은 스텁이 필요하다.**
  - **bun에선 `await browser.close()`가 resolve하지 않는다**(Chrome은 죽는데 프로세스가 7분 넘게 매달렸다). 닫기는 3초 race로 감싸고 끝에 `process.exit(0)`를 둔다 — 안 그러면 배치 루프가 매 실행 타임아웃을 기다린다.
...
- **Conventional Commits 필수**: `type(scope): 설명` 형식. 설명은 한국어/영어 모두 허용(레포 혼용).
  - **type**: `feat` · `fix` · `docs` · `perf` · `chore` · `refactor` · `test` (관찰된 어휘 — 새 type은 신중히)
...
- **자동 semver 릴리즈**(#595, claude-kit `Lyainc/claude-kit` 선례) — `.github/workflows/auto-release.yml` + `scripts/next-version.mjs`. 트리거는 둘: 머지된 PR의 `release` 라벨(주 트리거, 항상 릴리즈) 또는 미릴리즈 user-visible 커밋 70개(백스톱, 라벨 무관). user-visible은 `feat`/`fix`/`perf`/`refactor`+breaking뿐이고 `docs`/`chore`/`test`/`ci`/`build`/`style`은 단독으로 안 일으키되 다음 릴리즈에 실려간다. bump는 breaking>feat(minor)>그외(patch) 중 가장 큰 변화가 이긴다. **`package.json`의 `version`은 올리지 않는다**(`private:true`라 배포·설치 어디에도 안 쓰여 태그와 이중 관리할 값이 없다) — 그래서 이 워크플로는 main에 커밋을 push하지 않고 태그+`gh release create --generate-notes`만 한다(claude-kit의 release.yml이 겪던 "bump 커밋이 push 트리거를 재발화" 문제 자체가 없다).
...
- **TMDB 인앱 포스터 검색은 철거됐다(#665, 2026-08-10) — 되살리지 말 것.** #537이 넣었던 `TmdbPosterModal`·`/api/tmdb/{search,images,image}` 프록시·`checkTmdbRateLimit`/`checkTmdbImageRateLimit`(#638이 썸네일/원본으로 쪼갠 스코프 분리 포함)·TMDB API 키 환경변수가 전부 사라졌다. 근거는 TMDB API 약관 세 조항(AI 애플리케이션 금지·파생물 금지·상업적 사용은 별도 서면 동의)이고 **어느 하나도 코드로 못 푼다** — 특히 `docs/specs/landing-mood-first-ocr-entry.yaml`이 2차로 그리던 "OCR 제목 → 포스터 자동 검색" 체인이 1번 조항의 정면이라, TMDB를 안고 가면 그 동선을 영영 못 만든다(원문 대조는 `docs/NATIVE_IOS_RFC.md` §7.1·§9). **KOBIS는 그대로다** — 영화 메타데이터 축은 KOBIS가 계속 담당하고, 사라진 건 포스터 이미지를 외부에서 가져오는 기능 하나다. 포스터는 사용자 직접 업로드가 주 경로이고, 포스터 없이 시작하는 길은 #631이 열어놨다.
- **철거하면서 일부러 남긴 두 가지(#665).** ① 랜딩 이탈 경로가 3종 → 2종이 되면서 문구를 쌍으로 다시 잡았다: "포스터 있으면 올리기 · 포스터 없이 직접 입력" — TMDB가 맡던 "파일을 직접 못 구한 사용자"를 남은 둘이 나눠 받는다는 게 링크 이름만으로 읽혀야 해서다(테스트가 이 문구를 정확히 기대하므로 바꾸려면 같이 볼 것). ② `usePhototicket.fillEmptyMovieInfo`와 `__tests__/fillEmptyMovieInfoRatingZero.test.tsx`는 **호출부가 0인 채로 남겼다** — "이미 사용자가 채운 필드는 안 덮는다"와 그 회귀(숫자 0을 빈 값으로 오판하지 않기, #638 P2)는 다음 자동 보강 경로에서 다시 필요하다. 미사용 export라고 지우지 말 것.
- **Next Up**: #631(포스터를 선택 사항으로) 닫힘, PR #632 머지 완료(2026-08-02) — `_shared.tsx`의 `data-poster-root` 분기 + `canExport`에서 `hasPoster` 게이트 제거(D3)까지 전부 main에 있다. 진입 동선 재설계 에픽 #612가 착수 가능한 상태고, Seed spec `docs/specs/landing-mood-first-ocr-entry.yaml`이 1차 범위를 #615(히어로 캐러셀 전시 → 선택 재설계)·#635(OCR을 랜딩 주 CTA로 승격 + 이탈 경로 3종 → #665로 2종)로 확정해뒀다. 그 스펙의 2차였던 "OCR 제목 → 포스터 자동 검색 체인"은 #665로 폐기됐다(위 TMDB 항목) — 다시 하려면 TMDB가 아닌 소스로 처음부터 설계해야 한다.
