...
- **API route**: `src/pages/api/ocr.ts` — base64 JSON 수신(multipart 아님, `bodyParser.sizeLimit: '15mb'`). 가드 순서 method → 입력(MIME 화이트리스트·10MB) → rate limit → 인증 → 모델. **절대 throw 안 함**, 모든 에러를 status + `{ error }`로. Zod 스키마는 전 필드 `.nullable()`(`.optional()` 아님 — structured output의 `NoObjectGeneratedError` 회피). `chain`은 enum(에셋 슬러그와 1:1이라 4종으로 닫힘), **`format`은 자유 문자열**(#348 — 포맷 브랜드가 계속 늘어나고 목적지 `formatLabel`이 이미 자유 텍스트라 enum으로 닫으면 브랜드마다 코드를 고쳐야 한다. 표준 토큰은 스키마가 아니라 프롬프트가 좁힌다).
- **System prompt**: 지시문 영어 + 예시 한국어. 규칙이 전부 실측에서 나왔으니 **가볍게 고치지 말 것**(#125 A/B 15장 기준 STRICT 100%) — theater/screen 분리(CGV 앱은 지점명 줄 바로 아래 상영관 줄이 붙어서 "전도연관"을 지점명으로 오인함), 지점명 축약 금지, **로고 없는 CGV 티켓의 chain 판별**(CGV 앱 스크린샷엔 로고가 없어서 "판매번호" 라벨 + `연도-월일-4자리-4자리` 형식이 유일한 단서), 심야 상영 `25:00` 표기 보존. **format 규칙도 같은 15장 실측**(#348, format STRICT 15/15) — 포맷은 상영관 줄 안에 섞여 찍히고(`IMAX관`·`6관 (Laser)`·`MEGA | LED 3관`), CGV 배지는 IMAX/4DX일 때만 떠서(Laser관 배지는 `2D`) 배지만 보면 놓친다. 특별관·좌석등급 브랜드(전도연관·[CGV아트하우스]·디즈니시네마·르 리클라이너·광음시네마)는 포맷이 아니라 제외 — 어휘를 영사/음향 포맷으로 좁혀야 채점이 결정적이다.
