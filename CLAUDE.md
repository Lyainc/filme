...
- **TMDB는 살아 있다 — "폐기" 서술은 2026-08-02부로 철회됐다.** API 키를 발급받았고 인앱 포스터 검색이 #537(열림, Seed spec `docs/specs/tmdb-poster-search.yaml`)로 서 있다. KOBIS가 포스터를 안 주는 게 그 근거고(`docs/KOBIS_API.md`), 착수 시 열린 질문 1번(TMDB 포스터는 100% 2:3 세로인데 `stub`·`35mm-landscape`는 3:2 가로 슬롯 — #529 프리셋과 정면 충돌)을 먼저 정할 것. **폐기가 유지되는 건 Supabase뿐이다.**
