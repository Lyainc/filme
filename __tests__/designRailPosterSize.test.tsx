 ...
 *     크롭 파이프라인 자체는 셸이 소유하므로 여기서 검증할 건 "그 진입점이 실제로 불리는가"
 *     하나뿐이다. #607 전엔 모바일 rail·데스크톱 패널 두 화면을 describe.each로 같이 돌렸는데,
 *     DesktopDesignPanel이 삭제되며 DesignRail 한 화면만 남았다(항목 정의는 원래부터
 *     designRailItems.tsx 공용이라 커버리지 손실은 없다).
 ...
 *     desktopDesignPanel.test.tsx(f, #607에서 삭제)는 state 반영만 검증했다 — 그 사이(UI 조작이 만든 상태가
 *     실제 티켓 픽셀로 같은 결과를 내는가)가 이 테스트다. UI 경로가 posterFit 말고 다른
 *     필드까지 건드리면 마크업이 갈리며 여기서 잡힌다.
 *
 * 셋업은 designRail.test.tsx 미러 — 실제 usePhototicket을
...
function Harness({ onRecropPoster }: { onRecropPoster?: () => void }) {
  const photo = usePhototicket();
  latestComponents = photo.state.components;
  return <DesignRail photo={photo} onRecropPoster={onRecropPoster} />;
}

// rail은 한 번에 한 섹션만 펼치므로 '크기' 아이콘을 먼저 눌러야 본문이 살아난다
// (접힘 패널은 inert라 클릭이 안 먹는다).
const openSize = (user: ReturnType<typeof userEvent.setup>) =>
  user.click(screen.getByRole('button', { name: '크기' }));
...
describe('크기 섹션 포스터 크롭 (#492)', () => {
  test('(a) "포스터 다시 크롭" 클릭 → 셸의 재크롭 진입점이 1회 호출된다', async () => {
    const user = userEvent.setup();
    let calls = 0;
    render(<Harness onRecropPoster={() => { calls++; }} />);
    await openSize(user);
