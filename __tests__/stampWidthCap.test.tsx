 ...
 * P1). 4/5 무드는 여유가 커 scale=1.3에서도 예산 안에 든다 — 회귀 마커로 남긴다. Minimal만
 * edge/gaps/margin이 빡빡해(원래 예산도 avail에 딱 맞춰 설계됨, 위 describe 참고) 두 스탬프가
 * 동시에 STAMP_MAX_ASPECT(5:1)에 근접하는 로고를 올리고 scale을 최대(1.3)로 올리면 그룹 폭이
 * avail을 실제로 넘는다(≈134px 초과). 이 조합을 막으려면 클램프 공식(spec c4, "scale 적용 후 h
 * 기준 유지"— 하드 제약)이나 scale 범위(spec c5, 0.6~1.3 — 오너 확정)를 바꿔야 하는데 둘 다
 * 이 PR 스코프에서 임의로 바꿀 대상이 아니다. 실사용 로고가 5:1 근접 종횡비인 경우는 드물고
 * (체인은 enum 4종, 포맷도 브랜드 워드마크가 대부분) 슬라이더 두 개를 동시에 최댓값으로 올려야
 * 재현되는 이중 극단 조합이라, 자동 수정 대신 알려진 한계로 명시하고 owner 후속 판단에 넘긴다.
 */
describe('#441 scale(1.3) 확장 후 스탬프 그룹 폭 예산', () => {
  ...
  test('Minimal: scale=1.3 + 양쪽 최대종횡비 로고는 예산을 넘을 수 있음(알려진 한계, 자동 수정 안 함)', () => {
    const b = BUDGET.find((x) => x.mood === 'Minimal')!;
    const used =
      b.edge + b.chainH * MAX_SCALE * STAMP_MAX_ASPECT + b.gaps + b.formatH * MAX_SCALE * STAMP_MAX_ASPECT + b.margin;
    expect(used).toBeGreaterThan(b.avail);
  });
});
