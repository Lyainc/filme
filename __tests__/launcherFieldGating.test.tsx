...
import { MOOD_COMPONENTS } from './setup/moods';
...
function renderedFields(layout: LayoutId): TicketField[] {
  const Mood = MOOD_COMPONENTS[layout];
...
describe('launcherGroupsFor 게이팅 로직 (#287)', () => {
  ...
  // #558이 Criterion까지 전 무드 런처에서 뺐던 quote를 #777이 Criterion에만 되돌렸다 — 작은 화면에서
  // 온티켓 탭이 어려울 때 항목 목록이 대체 경로라서다. 다른 무드는 한줄평을 렌더하지 않아 그대로 제외.
  test('#777 — 한줄평은 실제 렌더하는 Criterion의 목록에서만 편집한다', () => {
  ...
  test('제외해도 빈 그룹만 사라지고 남는 필드는 순서 보존', () => {
    // Minimal·Criterion(둘 다 bookingNo만 제외) 모두 두 그룹이 다 남는다.
    expect(launcherGroupsFor('minimal').map((g) => g.title)).toEqual(['Film', 'Optional']);
    expect(launcherFields('criterion')).toContain('watchTime'); // #524 v5 — 콜로폰 1행이 관람시간을 렌더해 제외 해제
...
describe('6무드 죽은 컨트롤 0건 — 런처 필드 = 무드 렌더 필드 (#287)', () => {
  for (const layout of Object.keys(MOOD_COMPONENTS) as LayoutId[]) {
    test(`${layout}: 런처가 노출하는 필드 = 무드가 실제 렌더하는 필드`, () => {
      const rendered = renderedFields(layout);
