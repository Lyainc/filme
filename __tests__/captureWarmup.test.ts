...
describe('#439 blob: 포스터 — 캡처 시점 새 blob: 치환 + 캡처 후 원복', () => {
  // happy-dom은 canvas 2D를 기본 지원 안 해(getContext('2d') → null) blobSrcToObjectUrl이
  // 조용히 null을 리턴하고 스킵한다. 실제 치환 경로를 검증하려면 getContext/toBlob을 스텁한다.
  // data:가 아니라 blob:로 치환하는 이유(html-to-image의 embedImageNode가 data: src를 "이미
  // 임베드됨"으로 보고 decode 대기를 건너뛰는 레이스)는 captureToImage.ts의 blobSrcToObjectUrl
  // 주석 참고 — 이전엔 canvas.toDataURL()로 data: URL을 만들어 img.src에 심었었다.
  ...
  beforeEach(() => {
    ...
    HTMLCanvasElement.prototype.toBlob = ((callback: BlobCallback, type?: string) => {
      callback(new Blob(['AAAA'], { type: type ?? 'image/png' }));
  ...
  test('본 캡처(ratio 2) 순간엔 img.src가 새 blob:이고, 캡처 후 원래 blob:로 복원된다', async () => {
    ...
    // 본 캡처(ratio 2)는 새로 만든 blob: URL로 치환된 상태로 toJpeg에 전달된다(원본과 다른 URL).
    const mainCall = calls.find((c) => c.pixelRatio === 2);
    expect(mainCall?.imgSrc?.startsWith('blob:')).toBe(true);
    expect(mainCall?.imgSrc).not.toBe('blob:378');
...
describe('#439 후보④ — blob→blob: 캔버스 크기 상한 + 중복 src dedup', () => {
  // 위 블록과 같은 canvas 스텁이지만, toBlob 호출 시 this(canvas)의 width/height까지 기록해
  // 다운스케일 여부·호출 횟수를 검증한다.
  let originalGetContext: typeof HTMLCanvasElement.prototype.getContext;
  let originalToBlob: typeof HTMLCanvasElement.prototype.toBlob;
  ...
  beforeEach(() => {
    ...
    HTMLCanvasElement.prototype.toBlob = function (this: HTMLCanvasElement, callback: BlobCallback, type?: string) {
      ...
      callback(new Blob(['AAAA'], { type: type ?? 'image/jpeg' }));
    } as typeof HTMLCanvasElement.prototype.toBlob;
  });
  ...
  test('naturalWidth/Height가 상한(2048)을 넘으면 종횡비를 유지한 채 캔버스를 다운스케일한다', async () => {
    ...
    // 워밍업(ratio 1)은 blobSrcToObjectUrl을 안 거치므로 본 캡처 직전 1회만 toBlob이 불린다.
  ...
  test('같은 blob src를 공유하는 두 <img>(contain fit의 배경+전경 포스터)는 캔버스 인코딩을 한 번만 한다', async () => {
    ...
    // 두 img가 같은 src를 공유하므로 blobSrcToObjectUrl은 한 번만 실행돼야 한다.
    expect(canvasCalls.length).toBe(1);
    // 둘 다 캐시된 같은 새 blob: URL로 치환된 채 본 캡처(ratio 2)에 전달된다.
    const mainCall = calls.find((c) => c.pixelRatio === 2);
    expect(mainCall?.imgSrc?.startsWith('blob:')).toBe(true);
