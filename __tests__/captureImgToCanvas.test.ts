...
// html-to-image의 toJpeg에 실제로 뭐가 전달되는지(img vs canvas, 크기)를 검증한다.
// mock.module은 hoisting 안 됨 — 등록 후 require로 SUT를 가져와야 가로채진다(CLAUDE.md).
let calls: Array<{ imgSrc?: string; canvases: Array<{ width: number; height: number; styleFilter: string }> }> = [];
mock.module('html-to-image', () => ({
  toJpeg: (_node: unknown) => {
    ...
    const canvases = Array.from(el.querySelectorAll?.('canvas') ?? []).map((c) => ({
      ...
      styleFilter: (c as HTMLCanvasElement).style.filter,
    }));
    calls.push({ imgSrc: img?.src, canvases });
...
describe('#439 blob: 포스터 — 캡처 시점 <canvas> 노드 치환 + 캡처 후 원복', () => {
  ...
  test('캡처는 <img> 대신 <canvas>가 들어간 상태로 toJpeg에 전달되고, 끝나면 원래 <img>로 복원된다', async () => {
    ...
    expect(calls[0].imgSrc).toBeUndefined();
    expect(calls[0].canvases.length).toBe(1);
  ...
  test('getContext 실패(캔버스 미지원)면 원래 <img> 그대로 캡처된다', async () => {
    ...
    expect(calls[0].imgSrc).toBe('blob:nofallback');
    expect(calls[0].canvases.length).toBe(0);
  });

  test('blob: 소스가 아닌 <img>(예: http URL)는 치환하지 않는다', async () => {
    const node = nodeWithPoster('https://example.com/x.png');

    await captureNodeToJpeg(node, OPTS);

    expect(calls[0].imgSrc).toBe('https://example.com/x.png');
    expect(calls[0].canvases.length).toBe(0);
  });
});
...
describe('#439 후보④ — blob <img> → <canvas> 노드 치환, 캔버스 크기 상한', () => {
  ...
  test('naturalWidth/Height가 상한(2048)을 넘으면 종횡비를 유지한 채 캔버스를 다운스케일한다', async () => {
    ...
    expect(calls[0].canvases).toEqual([{ width: 2048, height: Math.round(2301 * (2048 / 3068)), styleFilter: 'none' }]);
  });

  test('상한 이하 이미지는 원본 해상도 그대로 캔버스에 그린다', async () => {
    await captureNodeToJpeg(nodeWithPoster('blob:normal'), OPTS); // 기본 100x100

    expect(calls[0].canvases).toEqual([{ width: 100, height: 100, styleFilter: 'none' }]);
  });
  ...
  test('같은 blob src를 공유하는 두 <img>(contain fit의 배경+전경 포스터)는 각각 독립된 <canvas>로 치환된다', async () => {
    ...
    expect(calls[0].canvases.length).toBe(2);
...
describe('#439 — 포스터 CSS filter를 캔버스 픽셀에 구워 CSS filter로 안 남긴다', () => {
  // 노드 치환까지 해도 실기기에서 포스터만 계속 빈 채로 재현됐다 — 포스터 <img>에만 늘 걸리는
  // filter(saturate·contrast·brightness, 배경은 blur까지)가 WebKit의 foreignObject 안에서 합성이
  // 실패하는 게 아닌지 의심해, ctx.filter로 그리는 시점에 픽셀에 직접 구워버리고 캔버스 자체엔
  // CSS filter를 안 남기게 했다(captureToImage.ts blobImgToCanvas 주석 참고).
  let originalGetContext: typeof HTMLCanvasElement.prototype.getContext;
  let ctxFilterAtDraw: string | undefined;

  beforeEach(() => {
    calls = [];
    ctxFilterAtDraw = undefined;
    originalGetContext = HTMLCanvasElement.prototype.getContext;
    const fakeCtx = {
      drawImage: function (this: { filter?: string }) {
        ctxFilterAtDraw = this.filter;
      },
  ...
  test('img에 filter가 있으면 drawImage 전에 ctx.filter로 세팅되고, 캔버스 style.filter는 none이다', async () => {
    const node = nodeWithPoster('blob:filtered');
    const img = node.querySelector('img')!;
    img.style.filter = 'saturate(0.92) contrast(1.05) brightness(0.5)';
    img.dataset.role = 'poster';

    await captureNodeToJpeg(node, OPTS);

    expect(ctxFilterAtDraw).toBe('saturate(0.92) contrast(1.05) brightness(0.5)');
    // 픽셀에 구웠으니 캔버스 자체엔 CSS filter를 안 남긴다(이중 적용 방지).
    expect(calls[0].canvases[0].styleFilter).toBe('none');
  });

  test('img에 filter가 없으면 ctx.filter를 건드리지 않는다', async () => {
    const node = nodeWithPoster('blob:nofilter');

    await captureNodeToJpeg(node, OPTS);

    expect(ctxFilterAtDraw).toBeUndefined();
  });
});
