...
describe('MoodStub SCREEN 셀 분해 ghost (#266 PR-B)', () => {
  ...
  // flex여야 줄바꿈 없이 한 줄 정렬된다. 마스터 재동기화(#281)로 푸터가 gap:10px를 정직하게 쓰므로,
  // HALL 혼합-ghost flex 시그니처는 gap:14px로 옮겼다(스텁 내 다른 gap과 겹치지 않는 유일값).
  test('theater 값 + 빈 screen + ghost=true → 텍스트·ghost 혼합을 flex로 정렬', () => {
    ...
    expect(html).toContain('gap:14px'); // flex 컨테이너 — 없으면 block ghost가 줄바꿈돼 깨짐
  });
