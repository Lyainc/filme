...
- `backdrop-tiles.webp` — 배경 타일 그리드 합성 시트(#613 배경 시트 항목). `Landing.tsx`의
  ...
  식별 불가)를 자산 자체로 만족한다. 400×675, 다크 테마 기준(헤드리스 Chrome 기본값)으로 구웠다 —
  현재는 아직 어디서도 소비하지 않는다(`Landing.tsx`는 여전히 라이브 div를 그린다, 라이브 렌더가
  테마별 대비를 스스로 맞추기 때문). 이 파일을 `<img>`로 실제 연결하는 건 별도 작업이다.
