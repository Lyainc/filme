...
극장 체인·상영 포맷 로고는 저작권 문제로 저장소에 번들하지 않아요. 사용자가 `FieldAccordion`/`FieldLauncher` → `FieldEditorBody`의 StampSheet에서 로고를 직접 업로드(useLogoCrop 자유 크롭)해요. 구 파일명 기반 매니페스트 파이프라인(`generate-asset-manifest.ts` → `assets.generated.ts`, predev/prebuild hook)과 `TheaterChainPicker`/`FormatPicker`는 #231·#196에서 제거됐어요.
