...
극장 체인·상영 포맷 로고는 저작권 문제로 저장소에 번들하지 않아요. 사용자가 `FieldAccordion`(데스크톱)/온-티켓 탭→`FieldEditSheet`(모바일) → `FieldEditorBody`의 StampSheet에서 로고를 직접 업로드(useLogoCrop 자유 크롭)해요. 구 파일명 기반 매니페스트 파이프라인(`generate-asset-manifest.ts` → `assets.generated.ts`, predev/prebuild hook)과 `TheaterChainPicker`/`FormatPicker`는 #231·#196에서 제거됐어요.

`ChainStamp` / `FormatStamp`은 업로드된 로고가 없으면 **null을 렌더**해요. raw value가 티켓에 새어 나가지 않도록 안전망.
