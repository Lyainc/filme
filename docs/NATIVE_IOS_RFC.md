...
불안정성 자체는 고쳤다(#660, 실측 근거로 @2x variant에 CI 기준선 위 명시 timeout 부여) — 단
이건 "그물이 튼튼한가"를 고친 것이지 "그물이 몇 겹이어야 하는가"는 그대로다.
...
- **`#660` 픽셀 diff 테스트 안정화** — @2x variant에 실측 근거 댄 명시 timeout 부여
