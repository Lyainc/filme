...
export type OcrResult = Partial<MovieInfo> & {
  ...
  rateLimited?: boolean;
  /** HTTP/네트워크/파싱 실패 — 정상 빈 인식 결과와 구분한다(#773). */
  failed?: boolean;
};
...
export async function runOcr(file: File): Promise<OcrResult> {
  ...
  try {
    const { base64, mimeType } = await prepareImagePayload(file);
    ...
    if (!res.ok) return res.status === 429 ? { rateLimited: true } : { failed: true };

    // 서버는 채워진 string 필드만 담은 객체를 반환한다. 무검증 캐스트 대신
    // 최소한 객체 형태인지 확인하고, 아니면 실패 표시를 돌려준다.
