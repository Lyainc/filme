/**
 * OCR 전처리 유틸 — 티켓 스크린샷을 vision 모델에 넘기기 전에
 * 크기와 노이즈를 줄여 인식 정확도를 높이고 이미지 토큰 비용을 절감한다.
 *
 * 처리 파이프라인 (순서 고정):
 *   1. 하단 18% 크롭  — 앱 UI 버튼·고지 영역 제거
 *   2. width 512 캡   — 이미지 토큰 절감 (width가 이미 ≤512 이면 스킵)
 *   3. JPEG 92% 재인코딩
 *
 * 폭 캡 512는 gpt-4o-mini(detail=high, 512px 타일 과금) 시절에 정해졌지만(#111),
 * 모델이 gemini-3.1-flash-lite로 바뀐 뒤에도 그대로 유지한다 — 실 티켓 15장 A/B에서
 * 폭 512로 STRICT 100%(예매번호·좌석 같은 작은 글씨 포함 15/15)라 더 키울 이유가 없다(#125).
 */
