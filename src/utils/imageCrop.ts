import { posterOutputSize } from './constants';
...
export interface CropOutputOptions {
   ...
   * (`posterOutputSize` — 960×1440 또는 가로 1440×960).
   */
  maxSide?: number;
}
 ...
 *
 * 기본(opts 없음)은 포스터 표준 경로 — `posterOutputSize`(960×1440 / 가로 1440×960), image/jpeg 0.95.
...
export async function getCroppedImg(
  ...
  let outH: number;
  if (maxSide == null) {
    // 포스터 표준 크롭: 크롭 프레임이 프리셋 비율로 잠겨 있어 그리기에서 늘어나지 않는다 —
    // 캔버스 비율(0.626)로 뽑던 옛 1534는 0.667 크롭을 세로로 잡아늘였다.
    ({ width: outW, height: outH } = posterOutputSize(pixelCrop));
  } else {
