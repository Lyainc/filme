import { POSTER_EDGE_FEATHER, parseObjectPosition, posterFeatherAxes } from './posterFeather';
import { TEXTURE_RECIPES, gradientBitmapSvg, isNoiseRecipe, noiseTileSvg, type TextureBlend } from './textureRecipes';
 ...
 * 1920×2752 ≈ 21MB, #526 ②). 이제 알파 1인 중앙은 메인 캔버스에 바로 그리고 밴드 둘(각 1922×48)만
 * 밴드 크기 오프스크린으로 얹는다 — 램프 모양·세기·씸 위치는 그대로다.
 *
 * 이득은 ctx.filter가 먹는 경로(데스크톱) 것이다. iOS는 색보정을 픽셀로 구워야 해서(#490/#495)
 * 중앙 draw가 drawImageColorFiltered 안에서 여전히 포스터 크기 오프스크린을 잡는데, 그건 페더가
 * 아니라 색보정에 딸린 비용이라 cover 경로도 똑같이 치른다 — 여기서 줄일 수 있는 몫이 아니다.
 *
...
function drawPosterFeathered(
  ...
  /**
   * 밴드 한 줄 — `[bandLo, bandHi)`(정수 device px) 구간을 밴드 크기 오프스크린에 그려 메인에
   ...
   */
  const band = (bandLo: number, bandHi: number, rampFrom: number, rampTo: number): void => {
    const thickness = bandHi - bandLo;
    if (thickness < 1) return;
    const bx = vertical ? crossFrom : bandLo;
    const by = vertical ? bandLo : crossFrom;
    const bw = vertical ? crossTo - crossFrom : thickness;
    const bh = vertical ? thickness : crossTo - crossFrom;
    ...
    // (#490/#495) 그린 뒤 픽셀로 굽는다 — 밴드 크기라 포스터 전체를 굽는 것의 1/30 수준이다.
    if (filter !== 'none' && !isCtxFilterHonored()) {
  ...
  band(Math.floor(start), near, start, start + f);
 ...
 */
/**
 * 후가공 오버레이 합성 한 단계를 실패 격리한다(#490/#495 후속, 재질·코팅 합성에
 * 일반화). save/restore로 감싸 실패 시 캔버스 상태를 통째로 되돌린다 — compositeOverlay는
 * 내부에서 ctx.save()→clip()→…→restore()로 감싸는데, 그 사이에서
 ...
 */
async function safeCompositeStep(
  ctx: CanvasRenderingContext2D,
  label: string,
  run: () => Promise<void>,
): Promise<void> {
  ...
  try {
    ...
    console.error(`[capture:overlay] FAILED ${label} — 이 오버레이만 건너뛴다`, err);
  } finally {
    ctx.restore();
  }
}
...
export async function captureNodeToJpeg(
  ...
  for (const root of posterRoots) {
    ...
    if (material) {
      ...
      await safeCompositeStep(ctx, `texture=${material}`, () =>
        compositeOverlay(ctx, root, material, intensity, nodeRect, width, height, pixelRatio, debug));
    }
