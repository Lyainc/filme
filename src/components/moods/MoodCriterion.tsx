...
/**
 * 배경 패턴(#530→#728) — 예전엔 캔버스 전면 레이어에서 도판 사각형을 evenodd로 파내는
 * PATTERN_CLIP이 필요했다(저장물에서 포스터가 raw canvas로 먼저 깔리고 도판의 불투명 배경이
 * base PNG에서 빠져 있어, 클립이 없으면 패턴이 그 구멍으로 비쳐 포스터를 덮었다). #728로
 * 패턴이 도판 왼쪽 여백(x84..214)에 고정된 좁은 박스가 되면서 도판(x230..730)과 원천적으로
 * 안 겹치게 됐고, 클립은 더 이상 필요 없다.
 */

/**
 * 헤더·평점 메타 조판(#575) — `UNE SÉANCE`, 관람일이 이 하나를 읽는다(평점 분모 `/5`는 #752로 제거됨).
...
export const MoodCriterion = memo(function MoodCriterion({ movieInfo: d, components, croppedImageUrl, fieldVisibility: fv, ghost, onField, onPosterTap, embossStamps, embossPaths, embossIntensity, reliefStamps, reliefPaths, reliefIntensity }: MoodProps) {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, background: PAPER_GRAIN, color: INK, fontFamily: FONT_KR, overflow: 'hidden' }}>
      {/* 배경 이미지(#530→#672→#728) — 루트의 첫 형제라 종이 그레인 바로 위, 도판·조판 **아래**에
          깔린다. componentOpacity 밖(종이에 이미 인쇄된 바탕)인 건 Editorial과 같은 계약이다.
          box는 도판 왼쪽 여백에 고정된 스탬프 자리라(위 PATTERN_CLIP 주석 참고) 도판과 안 겹친다. */}
      <BackgroundPatternLayer
        image={components.backgroundPatternImage}
        box={{ left: PAD, top: PLATE_TOP, width: PLATE_LEFT - PAD - 16, height: PLATE_H }}
      ...
      <div style={{ position: 'absolute', inset: 0, opacity: componentOpacity }}>
        ...
        {/* 한줄평 — 190px 고정 블록. 따옴표는 문구 길이와 무관하게 좌상·우하에 고정된다.
            ...
            사라졌다): 텍스트 폭 = 960 − PAD 84×2 − 인셋 96×2 = 600px.

            **줄 높이는 폰트마다 다르다** — 옛 주석의 "폰트 무관, 항상 64px"는 LATIN_SIZE_SCALE
            (`_shared.tsx`, #437)이 붙기 전 가정이 그대로 남은 오기였다. userTextFont가 라틴
            텍스트에 baseFontSize 50 × 폰트별 배율을 곱해 실제 fontSize를 정하므로(hand/brush/
            flower는 상한 1.25 걸려 62.5, gothic 50.1, batang 51.95, ink 57.15, eunyoung 61.9,
            coolguy 62.3), 2줄 높이(fontSize×1.28×2)도 128~160px로 갈린다 — 190px 블록 안에
            남는 여유는 62px(고딕류)부터 최소 30px(hand/brush/flower, fontSize 62.5)까지.

            폰트별 프리셋 폭 재실측(9택 전체, 브라우저 자연px·600px 슬롯·실제 userTextFont
            fontSize, 2026-09-06 — 이전 표는 4택 시절 3종만 커버했고 그중 손글씨 수치도
            LATIN_SIZE_SCALE 도입 전 flat fontSize 50 기준이라 지금 배율 1.25와 안 맞았다):
              폰트        | fontSize | 최장 프리셋(50자*) | 기본 quote(45자*)
              auto(라틴)  |  50      |  879.6px (1.47줄)  |  766.2px (1.28줄)
              gothic      |  50.1    | 1102.8px (1.84줄)  |  980.8px (1.63줄)
              **batang**  | **51.95**| **1125.1px (1.88줄)** | **983.3px (1.64줄)**
              hand        |  62.5    | 1025.1px (1.71줄)  |  881.8px (1.47줄)
              ink         |  57.15   |  937.4px (1.56줄)  |  812.4px (1.35줄)
              eunyoung    |  61.9    | 1037.8px (1.73줄)  |  894.1px (1.49줄)
              brush       |  62.5    | 1032.1px (1.72줄)  |  930.4px (1.55줄)
              coolguy     |  62.3    |  860.1px (1.43줄)  |  751.2px (1.25줄)
              flower      |  62.5    |  838.8px (1.40줄)  |  745.3px (1.24줄)
            (* 옛 주석은 49/44자라고 적었지만 현재 RATING_QUOTES/DEFAULT_QUOTE 실제 글자수는
            50/45자다 — 이후 문구 수정으로 하나씩 늘어난 채 안 갱신돼 있었다.)

            최악은 **고딕이 아니라 batang**(1125.1px=1.88줄, 3줄 기준 1200px까지 여유 6.2%) —
            #754가 다른 방법론(무공백 반복 em 비교)으로 이미 의심했던 걸 이번에 같은 잣대로
            확인. 실제 wrap 렌더(line-clamp 없이 줄바꿈만 건 높이) 기준으로도 9택×2문구 18개
            조합 전부 정확히 2줄에서 멈추고 3줄로 안 넘어간다(headless Chrome 실측) — 프리셋은
            9택 전체에서 안전하고, 클램프 스크린샷 확인이 따로 필요한 폰트는 없었다. 다만
            batang 여유(6.2%)가 옛 "고딕 최악, 여유 9%" 서술보다 빠듯하므로, 새 프리셋 문구
            길이나 fontSize·인셋을 건드릴 땐 batang을 기준으로 다시 잴 것.
