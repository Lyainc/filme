...
export const MoodMinimal = memo(function MoodMinimal({ movieInfo: d, components, croppedImageUrl, fieldVisibility: fv, ghost, onField, onPosterTap, embossStamps, embossPaths, embossIntensity, reliefStamps, reliefPaths, reliefIntensity }: MoodProps) {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, color: ink, fontFamily: FONT_SANS, overflow: 'hidden' }} {...posterTapProps(onPosterTap)}>
      <Poster
        src={croppedImageUrl}
        {...posterFitProps({ letterboxBg: posterBg, frameInsetY: POSTER_FRAME_INSET_Y, fit: components.posterFit })}
        ...
        reliefStamps={reliefStamps} reliefPaths={reliefPaths} reliefIntensity={reliefIntensity}
        ...
        <>
          ...
          <div style={{ position: 'absolute', left: 60, top: 52, display: 'flex', alignItems: 'center', gap: 34 }}>
            <StampRow
              ...
              chainHeight={74}
              chainScale={Math.min(components.chainScale ?? 1, MINIMAL_STAMP_MAX_SCALE)}
              format={components.format}
              formatLabel={components.formatLabel}
              formatVisible={components.formatVisible}
              formatSize={1.02}
              formatScale={Math.min(components.formatScale ?? 1, MINIMAL_STAMP_MAX_SCALE)}
              surface={stampSurface}
              ghost={ghost}
              onField={onField}
              dividerColor={ink}
