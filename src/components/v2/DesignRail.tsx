    ...
    >
      <div className="overflow-hidden" inert={!open || undefined}>
        ...
        {/* 고정 슬롯(#563) — 항목마다 콘텐츠 높이가 89~205px로 갈려서(400×675 실측) 탭을 옮길
            ...
            199는 넘침, #682 실측 — 그때 함께 넘치던 형압 패널은 #785에서 기능째 삭제됐다).
            26svh(393×659에서 171px)로 올려 컬러(둘 다)까지 들어오게 한다.
            크기 콘텐츠 다이어트(#682 후속)로 크기 패널의 정상 상태(축 전환 직후)는 171px 이하로
            들어왔다 — 남는 건 재크롭 직후 채우기 칩까지 겹치는 순간(179px, +8px) 하나다. 항목
            콘텐츠를 더 키우면 이 상한도 같이 올릴 것 — 안 올리면 조용히 스크롤이 돌아온다.
            railSlot은 게이트가 아니라 관측값(CLAUDE.md "📏 크롬 측정 하네스" 절)이라 exit code는
            원래부터 이 값에 안 실린다.
        ...
        <div
          ...
          // 크기가 위 다이어트 뒤에도 여전히 살짝 넘치는 잔여 상태(채우기 노출 +8px)에서
          // "더 있다"는 단서를 최소 침습으로 남긴다.
          // 그림자 색은 고정 rgba(0,0,0,…) 대신 --fg-faint(아이콘/장식 3:1 하한, 라이트
          // #818C99 / 다크 #6B7280)를 쓴다 — 다크 앰비언트(#0E1012 근방)에 검정 반투명을
          // 얹으면 대비가 거의 0이라 정작 남아 있는 넘침(크기)을 다크 테마에서만 못
