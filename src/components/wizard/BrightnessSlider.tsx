...
interface BrightnessSliderProps {
  ...
  max?: number;
  /**
   * 라벨 줄에 붙는 보조 텍스트 액션(#682 다이어트) — 이 슬라이더에 종속된 동작을 별도 전폭
   * 버튼(줄+간격 52px)으로 안 두고 라벨 옆에 접어, 슬라이더
   * 하나만큼의 세로 예산으로 둘을 같이 담는다. 옵션이라 기존 호출부는 전부 그대로다.
   * 로고 크기 패널의 "입력"·"편집"(#781)도 같은 자리를 쓴다. 보이는 글자가 한 단어라 어느 슬라이더의
   * 동작인지 접근명엔 슬라이더 라벨을 붙인다(`극장 로고 크기 — 입력`).
   */
