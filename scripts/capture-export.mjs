 ...
 *   bun scripts/capture-export.mjs --layout stub --full-fields --field-off seat,watchDate,watchTime,screen --ghost-off --out /tmp/stub-admission-off.jpg
...
/**
 * `--field-off <csv>`(#755 → #761) — 필드를 개별로 끄는 오버라이드. 나열한 필드만
 ...
 * HALL 행이 그대로 산다. theater는 admission 네 필드 목록에 없는 다섯 번째 필드라 독립 플래그를
 * 안 열고 screen에 묶는다. `releaseDate`(#761)는 admission이 아니라 filmOn 쪽 — base seed가
 * releaseDate:'2014'를 항상 심어서(fullFields 무관) --full-fields 없이도 filmOn이 releaseVal
 * 하나로 계속 true였는데, 이걸 끄면(runtime/rating/actors/reissue는 --full-fields 없이는 애초에
 * unset) filmOn을 admission은 안 건드리고 끌 수 있다 — Admission-only 조합을 만드는 유일한 길.
 */
const SUPPORTED_FIELD_OFF = ['seat', 'watchDate', 'watchTime', 'screen', 'releaseDate'];
function buildFieldVisibilityOverride(fieldOff) {
  const overrides = {};
  for (const f of fieldOff) {
    if (!SUPPORTED_FIELD_OFF.includes(f)) {
      throw new Error(`--field-off 지원 필드: ${f} (허용: ${SUPPORTED_FIELD_OFF.join(',')})`);
    }
    overrides[f] = false;
...
/**
 * stub 무드 전용 실측(#755 → #761) — PATTERN_BOX(MoodStub.tsx, 고정 y1060~1102)와 Admission/
 * The Film 섹션 헤드 사이 겹침을 판정하려면 두 헤드의 자연 픽셀(960 캔버스 기준) top이 필요하다.
 ...
 * Admission 헤드는 admissionOn이 false면 DOM에 없으므로 못 찾으면 null.
 */
async function measureStubFilmHead(page) {
  ...
  return page.evaluate(() => {
    ...
    const headRect = (text) => {
      ...
      if (!el) return null;
      const r = el.getBoundingClientRect();
      return { top: +((r.top - wRect.top) / scale).toFixed(1), bottom: +((r.bottom - wRect.top) / scale).toFixed(1) };
    };
    const film = headRect('The Film');
    const admission = headRect('Admission');
    if (!film && !admission) return null;
    return {
      filmHeadTop: film?.top ?? null,
      filmHeadBottom: film?.bottom ?? null,
      admissionHeadTop: admission?.top ?? null,
      admissionHeadBottom: admission?.bottom ?? null,
    };
  });
}

async function capture({ layout, material, coating, intensity, bg, bgScale, fullFields, fieldOff, ghostOff, emboss, lasso, relief, switchTo, toggleFill, out, timeoutMs }) {
  const seed = {
    movieInfo: {
      title: '인터스텔라',
  ...
  try {
    ...
    // clickByText와 같은 계약(찾아서 클릭, 없으면 던짐)을 CSS selector로 — 편집 메뉴 햄버거처럼
    // 텍스트가 아니라 aria-label로만 특정되는 요소(코드리뷰 지적: silent-swallow 방지 + 세 단계
    // 모두 같은 에러 처리로 통일).
    const clickBySelector = async (selector, label) => {
      ...
      if (!ok) throw new Error(`버튼을 못 찾음: ${label} (${selector})`);
    };
    ...
    mark('cropped');
    // `--ghost-off`(#761) — `--field-off`는 fieldVisibility만 끄는데, ghost 모드(MobileEditorShell
    // ghostMode, 기본 true)가 켜진 채면 showFieldGhost가 숨긴 필드에도 dim placeholder를 계속
    // 반환해 admissionOn/filmOn이 실제로는 안 꺼진다(gap-761 리뷰가 실측으로 확인) — PATTERN_BOX
    // 겹침처럼 "그 섹션이 진짜로 꺼진 상태"를 재려면 이 토글도 같이 꺼야 한다.
    if (ghostOff) {
      // `button[aria-label="편집 메뉴"]`로 태그까지 좁힌다 — 메뉴가 열리면 패널 div(#editor-menu-panel)도
      // 같은 aria-label을 써서(MobileEditorShell.tsx) 태그 없이 찾으면 DOM 순서에 우연히 기대게 된다
      // (코드리뷰 지적). '빈 항목' 텍스트 대신 ariaLabel="빈 항목 미리보기"로 매칭하는 것도 같은 이유 —
      // COPY_TONE_GUIDE 톤 정비로 라벨 문구가 바뀔 수 있지만 접근성 계약은 더 안정적이다.
      await clickBySelector('button[aria-label="편집 메뉴"]', '편집 메뉴 햄버거');
      await sleep(300);
      await clickBySelector('[aria-label="빈 항목 미리보기"]', '빈 항목 토글');
      await sleep(300);
      await clickBySelector('button[aria-label="편집 메뉴"]', '편집 메뉴 닫기(같은 버튼 재클릭)');
      await sleep(300);
      mark('ghostOff');
    }
...
if (cmpIdx >= 0) {
  ...
  await capture({
    ...
    ghostOff: argv.includes('--ghost-off'),
